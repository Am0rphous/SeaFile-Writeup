...
2. Error in the `seafile` container when connecting to the `seafile-mysql` container. The error was `Skip running setup-seafile-mysql.py because there is existing seafile-data folder. Waiting for mysql server to be ready: %s (2003, "Can't connect to MySQL server on 'db' ([Errno 111] Connection refused)")`
